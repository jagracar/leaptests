...
public class Star implements PConstants {
    ...
    public Star(PVector pos, PVector vel, BlackHole bh, PApplet parent) {
        this.pos = pos.get();
        this.vel = vel.get();
        this.bh = bh;
        this.parent = parent;
        ...
        calculateScreenPos(1f);

        // Create the image that will be used to draw the star
        radius = 6;
        img = this.parent.createImage((int) (6 * radius), (int) (6 * radius), ARGB);

        img.loadPixels();
        for (int y = 0; y < img.height; y++) {
            for (int x = 0; x < img.width; x++) {
                float relDistSq = (PApplet.sq(x - img.width / 2f) + PApplet.sq(y - img.height / 2f)) / PApplet.sq(radius);
                float grey = PApplet.max(0, 255 * (1 - relDistSq));
                float alpha = PApplet.max(0, PApplet.min(255, 255 * (1.2f - relDistSq)));
    ...
    public void update(float dt, int iterations, float zoom) {
        // Update the coordinates
        PVector acc = PVector.mult(pos, -bh.getMass() / PApplet.pow(pos.mag(), 3));
        ...
        calculateScreenPos(zoom);

        // Update the flares image
        updateFlaresImage();
    }

    protected void calculateScreenPos(float zoom) {
        PVector bhPos = bh.getPos();

        parent.pushMatrix();
        parent.translate(bhPos.x, bhPos.y, bhPos.z);
        parent.rotateX(TWO_PI * parent.mouseY / parent.height);
        parent.rotateY(TWO_PI * parent.mouseX / parent.width);
        parent.scale(zoom);
        parent.translate(pos.x, pos.y, pos.z);

        // Save the screen position
        screenPos = new PVector(parent.modelX(0, 0, 0), parent.modelY(0, 0, 0), parent.modelZ(0, 0, 0));
        parent.popMatrix();
    }
    ...
    protected void updateFlaresImage() {
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                float dist = PApplet.sqrt(PApplet.sq(x - width / 2f) + PApplet.sq(y - height / 2f));

                if (dist < 0.9 * radius) {
                    float relAng = (PApplet.atan2((float) (y - height / 2f), (float) (x - width / 2f)) + parent.noise(x)) / TWO_PI;
                    flaresImg.pixels[x + y * width] = parent.color(255 * parent.noise(0.1f * (dist - noiseSeed), 3 * relAng));
                }
            }
        }
        ...
        for (int y = 2; y < height - 2; y++) {
            for (int x = 2; x < width - 2; x++) {
                ...
                for (int i = -2; i < 3; i++) {
                    for (int j = -2; j < 3; j++) {
                        greySum += parent.red(flaresImg.pixels[x + i + (y + j) * width]);
                        counter++;
                    }
                }
    ...
    public void drawTrail(float zoom) {
        if (trail.size() > 4) {
            PVector bhPos = bh.getPos();
            ...
            for (int i = 0; i < trail.size() - 4; i++) {
                ...
                parent.line(trailPos.x, trailPos.y, trailPos.z, trailPos2.x, trailPos2.y, trailPos2.z);
            }
            parent.popStyle();
            parent.popMatrix();
        }
